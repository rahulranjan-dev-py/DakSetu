    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription className="flex flex-wrap gap-1.5 pt-1">
          <Badge variant="green">{t('loan.eligibleAfter', { n: result.loan.eligibleAfterYears })}</Badge>
          ...
          <tbody className="tabular">
            {shown.map((r) => (
              <tr key={r.year} className="border-t border-slate-100">
                ...
                <td className="py-1.5 text-right font-bold text-emerald-700">{formatINR(r.loanValue)}</td>
              </tr>
            ))}
          </tbody>
        </table>
